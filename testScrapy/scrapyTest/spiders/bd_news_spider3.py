...
class BDNewsSpider3(Spider):
    # 一个爬虫的标识，必须是唯一的
    name = "zhong_guo_jing_ji"
    
    # 这些都需要读取数据库来初始化
    citys = {}
    keywords = {}
    #raw_url = "http://news.baidu.com/ns?cl=2&s=1&tn=newsdy&ct1=0&ct=0&rn=50&class=0&q1=keyword&pn=0"
    raw_url = "http://news.baidu.com/ns?word=keyword&pn=0&cl=2&ct=0&tn=news&rn=50&ie=utf-8&bt=0&et=0&rsv_page=1"
    ...
    def start_requests(self):
        ...
        for x in city_join_keyword:
            request = scrapy.Request(self.raw_url.replace("keyword",x[0]+" site:(www.ce.cn/)"),callback=self.parse)
