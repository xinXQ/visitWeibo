...
class Spider4Shanghai(Spider):
    # 一个爬虫的标识，必须是唯一的
    name = "spider_for_shanghai"
    
    start_urls = ["http://www.shfda.gov.cn/gb/node2/yjj/xxgk/zfxxgk/zxxxgk/sp/index.html"]
    ...
    def parse(self, response):
        item_urls = response.xpath('//*[@id="main"]/div/div[2]/div/div/div[2]/ul/li/a/@href').extract()
        for item_url in item_urls:
            yield scrapy.Request("http://www.shfda.gov.cn/"+item_url,callback=self.parseItem)
        if len(item_urls) != 0:
